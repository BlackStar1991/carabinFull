...
class ModelExtensionShippingNexpress extends Model
{
    ...
    function getQuote($address)
    {
        ...
        $image = HTTP_SERVER.'image/catalog/shipping/cur_nexpress.jpg';
        ...
        if ($status)
        {
            ...
            $quote_data['nexpress'] = array(
                ...
                'cost' => $this->config->get('nexpress_cost'),
                'tax_class_id' => $this->config->get('nexpress_tax_class_id'),
                'text' => $this->currency->format($this->tax->calculate($this->config->get('nexpress_cost'), $this->config->get('nexpress_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
            );
