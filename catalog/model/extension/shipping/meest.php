...
class ModelExtensionShippingMeest extends Model
{
    ...
    function getQuote($address)
    {
        ...
        $image = HTTP_SERVER.'image/catalog/shipping/cur_meest.jpg';
        ...
        if ($status)
        {
            ...
            $quote_data['meest'] = array(
                ...
                'cost' => $this->config->get('meest_cost'),
                'tax_class_id' => $this->config->get('meest_tax_class_id'),
                'text' => $this->currency->format($this->tax->calculate($this->config->get('meest_cost'), $this->config->get('meest_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
            );
